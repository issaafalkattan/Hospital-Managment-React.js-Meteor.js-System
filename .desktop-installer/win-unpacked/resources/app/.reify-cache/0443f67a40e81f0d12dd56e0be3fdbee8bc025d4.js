"use strict";module.export({default:()=>Squirrel});var path;module.link('path',{default(v){path=v}},0);var spawnSync;module.link('child_process',{spawnSync(v){spawnSync=v}},1);var fs;module.link('fs-plus',{default(v){fs=v}},2);var os;module.link('os',{default(v){os=v}},3);var electron;module.link('electron',{default(v){electron=v}},4);/* eslint-disable import/no-dynamic-require */
...
class Squirrel {
    ...
    static setUpAutoUpdater(context) {
        if (context.settings.squirrel && context.settings.squirrel.autoUpdateFeedUrl && context.settings.squirrel.autoUpdateFeedUrl.trim() !== '') {
            ...
            let feed = context.settings.squirrel.autoUpdateFeedUrl;
            feed = feed.replace(':version', version);
            feed = feed.replace(':platform', platform);
            context.l.info(`setting autoupdate feed to url: ${feed}`);
